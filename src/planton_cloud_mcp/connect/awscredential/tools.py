...
from planton_cloud.cloud.planton.apis.search.v1.connect import (
    query_pb2_grpc as connect_grpc,
    io_pb2 as connect_io
)
from planton_cloud.cloud.planton.apis.commons.apiresource import (
    io_pb2 as apiresource_io
)
from planton_cloud.project.planton.shared.apiresourcekind import (
    api_resource_kind_pb2 as api_resource_kind_pb2
)
...
async def get_aws_credential(credential_id: str) -> dict[str, str]:
    ...
    try:
        ...
        request = apiresource_io.ApiResourceId(
        ...
        if not credentials['access_key_id'] or not credentials['secret_access_key']:
            raise Exception(f"Invalid credential data for credential ID '{credential_id}': missing access key or secret key")
...
async def list_aws_credentials() -> list[dict[str, Any]]:
    ...
    try:
        ...
        # Build the request
        request = connect_io.SearchCredentialApiResourcesByContextInput(
            org=org_id,
            api_resource_kind=api_resource_kind_pb2.aws_credential  # Filter for AwsCredential
        )
