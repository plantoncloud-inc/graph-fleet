...
def _import_tools() -> dict[str, Any]:
    ...
    try:
        ...
        from .connect.awscredential import (
            get_aws_credential,
            list_aws_credentials,
        )
        from .infra_hub.aws.aws_ecs_service import (
            get_aws_ecs_service,
        ...
        tools.update(
            {
                ...
                "get_aws_ecs_service_latest_stack_job": get_aws_ecs_service_latest_stack_job,
            }
        )

    except ImportError as import_error:
        # Log the import error and continue without tools
        print(f"Warning: Failed to import MCP tools: {import_error}")
        # Return empty tools dict - server can still run without tools
