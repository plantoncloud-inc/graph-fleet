...
from .credential_context import (
    extract_credentials_from_stack_job,
...
async def get_aws_credentials_context() -> str:
    ...
    try:
        ...
        masked_creds = {
            ...
            if len(credentials["access_key_id"]) > 10
            else "***",
...
async def extract_and_set_credentials_from_stack_job(stack_job_json: str) -> str:
    ...
    try:
        ...
        masked_info = {
            "access_key_id": credentials["access_key_id"][:10] + "...",
            "region": credentials.get("region", "unknown"),
            "provider_credential_id": stack_job.get("spec", {}).get(
                "provider_credential_id", "unknown"
            ),
        }

        return (
            f"Successfully extracted and set AWS credentials: {json.dumps(masked_info)}"
        )
...
def create_session_credential_tools(credential_context: Any) -> list[Any]:
    ...
    async def get_aws_credentials_context() -> str:
        ...
        try:
            ...
            if not credentials:
                return json.dumps(
                    {"error": "No AWS credentials found in session context"}
                )
    ...
    async def clear_credential_context() -> str:
        ...
        try:
            ...
            return (
                "Successfully cleared all credentials and context from session memory"
            )
