...
async def get_all_mcp_tools(
    ...
    tools = await get_mcp_tools(
        aws_credentials=aws_credentials, credential_context=credential_context
    )
...
async def create_ecs_deep_agent(
    model: str | LanguageModelLike = "claude-3-5-haiku-20241022",
    aws_credentials: dict[str, str] | None = None,
    credential_context: CredentialContext | None = None,
    **kwargs: Any,
) -> Any:
    ...
    if credential_context:
        ...
        logger.warning(
            "No credential context provided - using global singleton (SECURITY RISK!)"
        )
    ...
    logger.info(
        f"Total tools available: {len(tools)} (including {tool_count} credential management tools)"
    )

    # Define a post-model hook to ensure messages have content
    def ensure_message_content(state: Any) -> Any:
