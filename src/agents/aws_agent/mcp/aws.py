...
async def mint_sts_credentials(
    ...
    try:
        ...
        if missing_fields:
            raise ValueError(f"Incomplete STS credentials received. Missing: {missing_fields}")
        ...
        logger.info(f"Successfully minted STS credentials for credential_id: {credential_id}")
        return sts_data
...
async def create_aws_mcp_client(
    ...
    aws_config = {
        "aws_api": get_aws_mcp_config(aws_creds)
    }
...
async def mint_sts_and_get_aws_tools(
    ...
    client_manager.aws_client = await create_aws_mcp_client(client_manager, sts_credentials)
