...
class ECSDeepAgentState(TypedDict):
    ...
    conversation_context: Optional[Dict[str, Any]]  # Extracted context from context-extractor subagent
    extracted_parameters: Optional[Dict[str, Any]]  # ECS identifiers, problem descriptions, user intent
    
    # User preferences and constraints
    user_preferences: Optional[Dict[str, Any]]  # Risk tolerance, timing constraints, communication style
    
    # Conversation history tracking
    conversation_history: Optional[List[Dict[str, Any]]]  # Historical context across multiple interactions
    conversation_session_id: Optional[str]  # Session identifier for conversation continuity
    
    # Subagent coordination state
    active_subagent: Optional[str]  # Currently active subagent
    subagent_handoff_context: Optional[Dict[str, Any]]  # Context passed between subagents
    conversation_flow_state: Optional[str]  # Current phase: "context_extraction", "triage", "planning", "execution", "verification", "reporting"
    ...
    solution_confidence: Optional[float]  # Confidence level in proposed solutions (0.0-1.0)
    ...
    approval_history: Optional[List[Dict[str, Any]]]  # History of user approvals and decisions
    
    # Follow-up and iterative conversation support
    follow_up_questions: Optional[List[str]]  # Questions that need user clarification
    pending_clarifications: Optional[Dict[str, Any]]  # Information waiting for user input
    conversation_branch_point: Optional[Dict[str, Any]]  # State for handling conversation branches
    ...
    last_user_interaction: Optional[Dict[str, Any]]  # Timestamp and context of last user input
