"""Credential Context Manager for AWS ECS Troubleshooting Agent.
...
class CredentialContext:
    ...
    def __init__(self) -> None:
        ...
        self._service_context: dict[str, Any] | None = None
    ...
    async def get_aws_credentials(self) -> dict[str, str] | None:
    ...
    async def set_service_context(self, context: dict[str, Any]) -> None:
        ...
        async with self._lock:
            ...
            logger.info(
                f"Service context set for: {context.get('service_id', 'unknown')}"
            )

    async def get_service_context(self) -> dict[str, Any] | None:
    ...
    @asynccontextmanager
    async def temporary_credentials(self, credentials: dict[str, str]) -> Any:
...
_credential_context: CredentialContext | None = None
...
async def extract_credentials_from_stack_job(
    stack_job: dict[str, Any],
) -> dict[str, str] | None:
    ...
    try:
        ...
        from planton_cloud_mcp.connect.awscredential.tools import (
            get_aws_credential,  # type: ignore[import-untyped]
        )

        # Fetch the actual credentials
        credentials: dict[str, str] = await get_aws_credential(provider_credential_id)
