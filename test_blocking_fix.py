...
async def test_planton_cloud_mcp_import():
    ...
    try:
        ...
        assert hasattr(planton_cloud_mcp, "create_mcp_server"), (
            "create_mcp_server not exported"
        )
        assert hasattr(planton_cloud_mcp, "get_mcp_server"), (
            "get_mcp_server not exported"
        )
...
async def test_contextualizer_tools_loading():
    ...
    try:
        ...
        logger.info(
            f"✅ Contextualizer tools loaded successfully in {loading_time:.3f}s"
        )
...
async def main():
    ...
    if failed == 0:
        ...
        logger.info(
            "✅ No 'Blocking call to ScandirIterator.__next__' errors detected."
        )
        return True
    else:
